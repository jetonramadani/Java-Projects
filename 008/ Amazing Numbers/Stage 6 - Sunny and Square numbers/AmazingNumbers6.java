...
abstract class NumCheck6 {
    abstract boolean checkNumber(long number);
}
class NumException6 extends Throwable {
    String str;
    NumException6(String str) {
        this.str = str;
    }
    @Override
...
class AmazingNumber6Helper {
    private final Scanner sc;
    private long num;
    private final HashMap<String, NumCheck6> checkers;
    public AmazingNumber6Helper(Scanner sc) {
        this.sc = sc;
        checkers = new HashMap<>() {
            {
                put("EVEN", new NumCheck6() {
                    @Override
                    public boolean checkNumber(long number) {
                        return number % 2 == 0;
                    }
                });
                put("ODD", new NumCheck6() {
                    @Override
                    public boolean checkNumber(long number) {
                        return number % 2 == 1;
                    }
                });
                put("BUZZ", new NumCheck6() {
                    @Override
                    public boolean checkNumber(long number) {
                        return number % 7 == 0 || number % 10 == 7;
                    }
                });
                put("DUCK", new NumCheck6() {
                    @Override
                    public boolean checkNumber(long number) {
                        return Long.toString(number).contains("0");
                    }
                });
                put("PALINDROMIC", new NumCheck6() {
                    @Override
                    public boolean checkNumber(long number) {
    ...
    private byte processRequest() throws NumException6 {
        ...
        try {
            ...
            throw new NumException6("The first parameter should be a natural number or zero.");
        }
        if (num == 0) {
            return 0;
        }
        if (parts.length == 1) {
            ...
            try {
                ...
                throw new NumException6("The second parameter should be a natural number.");
            }
            return 2;
        }

    }
    private void formatProperty(String name, NumCheck6 check) {
    ...
    private void throwMutuallyExclusive(byte state, int type) throws NumException6 {
        ...
        if (state == 2) {
            ...
            throw new NumException6(sb.toString());
        }
    }
    private void printContainsOrNot(long total, String ...constraints) throws NumException6 {
        ...
        if (wrong.size() != 0) {
            if (wrong.size() == 1) {
                throw new NumException6("The property [" + wrong + "] is wrong.\n" +
                        "Available properties: " + checkers.keySet());
            } else {
                throw new NumException6("The properties [" + wrong + "] are wrong.\n" +
                        "Available properties: " + checkers.keySet());
            }
        }
...
public class AmazingNumbers6 {
    public static void main(String[] args) {
//        write your code here
        Scanner sc = new Scanner(System.in);
        AmazingNumber6Helper number = new AmazingNumber6Helper(sc);
        number.run();
    }
}